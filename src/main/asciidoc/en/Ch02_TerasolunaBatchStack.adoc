...
[WARNING]
.Standard error output of xstream
====
xstream version is changed to 1.4.10, {SB} reads Bean definition and the following message is output in standard error output while running job.
...
Currently, since XStream security settings cannot be done in {SB}, message output cannot be controlled.

When using XStream for functions (such as data linkage) other than reading the developed job Bean definition, it should be used such that the XML other than the trusted generation source should not be read using XStream.
