...
=== Exception occurrence in pre-processing in StepExecutionListener
When an exception occurs in pre-processing (``beforeStep`` method), whether to execute open/close of resource by model, changes. The case of exception occurrence in pre-processing is explained in each model.

Chunk model::
Since pre-processing is executed before opening the resource, the resource is not opened. +
Since the resource is closed even when the resource is not opened, it is necessary to note this case when implementing ``ItemReader``/``ItemWriter``.

Tasklet model::
In Tasklet model, open/close the resource explicitly by ``execute`` method. +
When an exception occurs in pre-processing, ``execute`` method is not executed so naturally open/close of resources is also not executed.

[[Ch04_Listener_HowToUse_JobAbort]]
=== Job abort in pre-processing(StepExecutionListener#beforeStep())
When the conditions for executing the job are not satisfied, you may want to abort the process before executing the job.

In such a case, the process can be aborted before job execution by throwing an exception in pre-processing (``beforeStep`` method). +
The case of where the following requirements are implemented is explained with the help of an example here.

. Verify the validity of start parameters of input and output files by ``beforeStep`` method defined by ``StepExecutionListener``. +
. Throw an exception if any of the start parameters is not specified.

However, in {batch5_shortname}, it is recommended to use ``JobParametersValidator`` for verifying the validity of start parameters.
Refer to <<Ch04_JobParameter.adoc#Ch04_JobParameter_HowToUse_ParamsValidation,"Verification of validity of parameters">> for verifying the validity of start parameters since easy to understand validity verification is used till the end as a sample for discontinuing pre-processing here.

Implementation example is shown below.

[source,java]
.Implementation example of StepExecutionListener that performs validity verification of start parameters
----
@Component
@Scope("step")
...
|(1)
|Specify the parameter to be referred using @Value annotation.

|(2)
|Throw an exception. +
In this example, own exception class is used by inheriting ``RuntimeException``.

|(3)
|Specify the parameter to be referred. +
The class that sets parameter is a unique class implementing ``ItemStreamReader`` and ``ItemStreamWriter`` respectively.
