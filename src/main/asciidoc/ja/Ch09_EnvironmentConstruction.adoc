...
eArtifactId=terasoluna-batch-archetype -DarchetypeVersion={batch5_version}
...
a-batch-archetype:{batch5_version}
[INFO] -------------------------------------------------------------------------
---
...
Root Directoryに``C:\xxx\terasoluna-batch-tutorial``を設定し、Projectsにorg.terasoluna.batchのpom.xmlが選択された状態で、 [Finish]を押下する。
