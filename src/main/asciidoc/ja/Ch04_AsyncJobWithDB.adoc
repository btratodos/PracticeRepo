...
[WARNING]
====
カラムの自動採番に対応しているデータベースについては、{batch5_shortname}のjarに同梱されているDDLにジョブ要求シーケンスは定義されていない。
...
そのため、{batch5_shortname}が提供しているSQLでは対応できない場合がある。 +
その場合は<<Ch04_AsyncJobWithDB_HowToExtend_CustomTable>>を参考に、
``BatchJobRequestMapper.xml``のSQLMapを再定義する必要がある。 +
提供しているSQLについては、{batch5_shortname}のjarに同梱されている``BatchJobRequestMapper.xml``を参照のこと。
